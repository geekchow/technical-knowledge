...
flowchart TD
    ...
    subgraph tooling["Meta (outside the categories)"]
        ...
        Recent["RECENT.md — generated recent-docs shortcut"]
        Scripts["scripts/ — index generators (gen-recent.py)"]
...
- **`RECENT.md` is generated by a script,** never by hand: `python3 scripts/gen-recent.py` (`--limit N` to go further back). It lists the newest added/updated docs, newest first, and deliberately skips `README.md`/`index.md` churn and pure file moves so it stays a shortcut rather than a changelog.
...
- **Refresh `RECENT.md` after the article commit lands, not before** — the generator reads *committed* history, so an article that is still unstaged will not appear in it. Commit the article first, then regenerate and commit the refreshed `RECENT.md`; an index refresh may ride along with the next article's commit rather than needing one of its own.
