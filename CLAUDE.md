...
- **Never put a Jira ticket id in an article filename.** When an article is generated from a Jira ticket, name the file for its *subject*, not its ticket: `countdownlatch-in-java.md`, not `kan159-160-countdownlatch.md`. A ticket id means nothing to a reader, is unsearchable by topic, and goes stale when the tracker changes. The same applies to the `# ` heading and any index entry. **Traceability lives in `.csdn-tools/jira-articles.json`** — the single source of truth for the ticket ↔ article mapping — never in the article body.
- **Never put a date in an article filename.** `springboot-startup-lifecycle.zh.md`, not `2026-09-17-springboot-startup-lifecycle.zh.md`. A date in the name is redundant — `LATEST.md` already derives the real authoring date from git history (`--follow`, so it survives renames) — and it is actively misleading once an article is revised, because the name still claims the day it was created. Filenames describe the *subject*; dates, ticket ids and other metadata live outside the name.
