...
- **A bare `- [x]` checkbox in a series index means "this version is written", not "published to CSDN."** Only an explicit `CSDN 已发布：<url>` line or a URL-map entry counts as published — do not infer publish state from the plain checkbox.
